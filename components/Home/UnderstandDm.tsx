"use client";
import React from "react";

function UnderstandDm() {
  return (
    <div>
      {/* Part 1  */}
      <div className="bg-understand-bg1 w-screen h-screen bg-cover bg bg-no-repeat pl-[110px] pr-[110px] flex flex-col justify-around">
        <div>
          <h1 className="text-[100px] text-bee-offwhite font-[800] leading-[1.1]">
            Understanding
            <br />
            Digital Marketing
          </h1>
        </div>
        <div>
          <h1 className="text-[32px] font-[600] text-bee-orange">
            The Importance of Digital Strategy & Building a Career:{" "}
          </h1>
          <p className="text-[20px] text-bee-offwhite">
            Digital strategy is the backbone of any successful business today.
            It defines how a brand will connect with its audience online, from
            social media presence and website design to email marketing and
            search engine optimization. This strategic approach drives brand
            awareness, customer engagement, and ultimately, revenue growth. As
            the digital world continues to evolve, the demand for skilled
            digital strategists is skyrocketing. Building a career in this field
            offers exciting opportunities for creativity, innovation, and
            impactful work that shapes the future of marketing.
          </p>
        </div>
      </div>
      {/* Part 2  */}
      <div className="bg-understand-bg2 bg-black/100 w-screen h-screen bg-cover bg bg-no-repeat pl-[110px] pr-[110px] flex flex-col justify-around">
        <div>
          <h1 className="text-[32px] font-[600] text-bee-orange">
            Creativity & Professionalism in Digital Marketing:
          </h1>
          <p className="text-[20px] text-bee-offwhite">
            Digital marketing demands a unique blend of creativity and
            professionalism. It&apos;s about more than just flashy ads;
            it&apos;s about crafting compelling narratives, developing
            innovative campaigns that resonate with target audiences, and
            delivering results-driven strategies. This balance of creativity and
            professionalism is crucial for success in the dynamic world of
            digital marketing.
          </p>
        </div>
        <div>
          <h1 className="text-[32px] font-[600] text-bee-orange">
            The Value of Learning from an Industry Leader:
          </h1>
          <p className="text-[20px] text-bee-offwhite">
            Studying with an industry leader in digital marketing provides
            invaluable insights and unparalleled expertise. These leaders
            possess a deep understanding of the industry&apos;s best practices,
            emerging trends, and the challenges faced by real-world businesses.
            Our guidance goes beyond textbook knowledge, offering real-world
            experiences, valuable industry connections, and mentorship that can
            significantly accelerate your career growth.
          </p>
        </div>
      </div>

      {/* Orange Bar bg  */}
      <div className="bg-bee-orange w-screen h-[135px]"></div>
    </div>
  );
}

export default UnderstandDm;
